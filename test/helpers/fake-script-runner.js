    ...
    if (env.APPEND_PROXY_SETTINGS === "true") {
      await fs.writeFile(
        ...
        [
          ...
          `  ${env.PUBLIC_SERVICE}:`,
          "    networks:",
          `      ${env.TRAEFIK_NETWORK_NAME}: null`,
          "    labels:",
          '      traefik.enable: "true"',
          `      traefik.docker.network: ${env.TRAEFIK_NETWORK_NAME}`,
          `      traefik.http.routers.${projectName}.rule: Host(\`${previewHost}\`)`,
          `      traefik.http.routers.${projectName}.entrypoints: web`,
          `      traefik.http.services.${projectName}.loadbalancer.server.port: "${env.PUBLIC_PORT}"`,
          "networks:",
          `  ${env.TRAEFIK_NETWORK_NAME}:`,
          "    external: true",
          `    name: ${env.TRAEFIK_NETWORK_NAME}`,
          "",
        ].join("\n"),
        "utf8",
      );
