...
- Destroys preview stacks with `docker compose down -v --remove-orphans` for PR/branch previews, but keeps volumes (`down --remove-orphans`) for the dedicated default-branch deployment.
